package com.maisi.video.obj.record;
...
public class ExtAudioRecorder {
            ...
            if (state == State.INITIALIZING) {
                if (configuration.isUncompressed()) {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
                        ...
                        // 采样�? SampleRate*NumberOfChannels*BitsPerSample/8
    ...
    /**
     * 重置录音，并设置 state �?{@link State#INITIALIZING}，如果当前状态为 {@link State#RECORDING}，将会停止录音�?
     * 这个方法不会抛出异常，但是会设置状�?�?{@link State#ERROR}
     */
    public void reset() {
        try {
    ...
    /**
     * �?��录音，并设置 state �?{@link State#RECORDING}。在调用这个方法前必须调�?{@link ExtAudioRecorder#prepare()} 方法
     */
    public void start() {
    ...
    /**
     * 停止录音，并设置 state �?{@link State#STOPPED}。如果要继续使用，则�?��调用 {@link #reset()} 方法
     *
     * @return 录音的时�?
     */
    public int stop() {
